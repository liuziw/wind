...
@Component
@Slf4j
public class ReadExcelManagerImpl implements ReadExcelManager {
    ...
    public ArrayList<String> getSheetList(String filePath) {
        ArrayList<String> sheetList = new ArrayList<>(0);
        try {
            ...
            throw CommonException.exception("读取失败",e);
        }
        return sheetList;
    }
    ...
    public ArrayList<ArrayList<String>> readExcel(String filePath) {
        return readExcel(filePath, 0, "1-", "1-");
    }
    ...
    public ArrayList<ArrayList<String>> readExcel(MultipartFile file, int sheetIndex,String row) {
        return readExcel(file, sheetIndex, row, "1-");
    }

    @Override
    public ArrayList<ArrayList<String>> readExcel(MultipartFile file) {
        return readExcel(file, 0, "1-", "1-");
    }
     ...
     * 读取Excel文件内容
     */
    private ArrayList<ArrayList<String>> readExcel(Sheet sheet, String rows, int[] cols) {
     ...
     * 获取连续行、列数据
     */
    private ArrayList<ArrayList<String>> getRowsValue(Sheet sheet, int startRow, int endRow,
